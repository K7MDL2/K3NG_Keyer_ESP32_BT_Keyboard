...
#if defined(FEATURE_LCD_LIQUIDCRYSTAL_I2C)
	#include <Wire.h>
	#include "LiquidCrystal_I2C.h"
#endif
...
// ------------------------   Pico  BT Keyboard Support Functions --------------------------------------------------
	...
	void check_last_bt_addr_in_EEPROM() {
		...
		if (x) {
			...
			debug_serial_port->println("No previously paired devices found");
		}
	}
...
//    #define ICON_WIDTH COLUMN_WIDTH
		...
		if (last_WPM != (int) configuration.wpm)  // wpm changed, update display
		{
		...
		static byte last_ptt_line_activated= 255;
		...
		static byte last_pause_sending_buffer = 254;
...
void service_display() {
	...
	if (screen_refresh_status == SCREEN_REFRESH_IN_PROGRESS) {
		if (x > lcd_scroll_buffer[y].length()){  // used to draw buffer on screen when pause type event started
			y++;  // move to new row
			if (y >= LCD_ROWS){   // ran out of screen rows, trigger scroll up
										...
										if (bt_keyboard_available()) {
											keystroke = bt_keyboard_read();
										...
										#endif
												{
														switch (keystroke)
														{
																case PS2_LEFTARROW : sidetone_adj(20); break;
																case PS2_RIGHTARROW : sidetone_adj(-20); break;
																...
																case PS2_Y_CTRL : {  // cyc;le thogh GPS serial port baud rates and restart comms
																	...
																	configuration.gps_baud = next_baud;
																		...
																		gps_serial_port.end();
																		gps_serial_port.begin(next_baud);
																		#else
...
void write_settings_to_eeprom(int initialize_eeprom) {
	...
	#elif defined(ARDUINO_RASPBERRY_PI_PICO_W) || defined(ARDUINO_RASPBERRY_PI_PICO) || defined(HARDWARE_ESP32_DEV)
...
void repeat_play_memory(PRIMARY_SERIAL_CLS * port_to_use) {
	...
	int memory_number = serial_get_number_input(2,0, (number_of_memories+1), port_to_use, RAISE_ERROR_MSG);
	#ifdef DEBUG_CHECK_SERIAL
...
byte play_memory(byte memory_number) {
	...
	for (int y = (memory_start(memory_number)); (y < (memory_end(memory_number)+1)); y++) {
		...
		if (keyer_machine_mode == KEYER_NORMAL) {
			...
			#ifdef FEATURE_BT_KEYBOARD
				#if !defined(USE_BT_TASK) && !defined(USE_CORE1)  // core1 on Pico assumes
					check_bt_keyboard();
				#endif
			#endif
			#if defined(FEATURE_PS2_KEYBOARD) || defined(FEATURE_BT_KEYBOARD) || defined(FEATURE_TOUCH_DISPLAY)
...
void initialize_i2c(void) {
	#if defined(ARDUINO_RASPBERRY_PI_PICO_W) || defined(ARDUINO_RASPBERRY_PI_PICO)
		#ifdef USE_WIRE1 // for i2c1 bus pins
...
//-------------------------------------------------------------------
	...
	void init_MCP23017(void) {	
		...
		#else
			// Setup CPU side GPIO interrupt for IntA
			//debug_serial_port->printf("MCP23107: Setup Interrupt on CPU pin %d\n", MCP23017_INTA_GPIO);
			pinMode (MCP23017_INTA_GPIO, INPUT);
			//debug_serial_port->printf("MCP23107: Attach Interrupt on CPU pin %d\n", MCP23017_INTA_GPIO);
			attachInterrupt(digitalPinToInterrupt(MCP23017_INTA_GPIO), paddle_intr_handler, CHANGE);
			//debug_serial_port->println(F("MCP23107: Interrupt Setup done"));
			mcp23017.clearInterrupts();
		#endif
...
void initialize_default_modes(){
	...
	myDelay(25);  // wait a little bit for the caps to charge up on the paddle lines
...
void initialize_gps_port(void) {
		...
		#if !defined(GPS_TEST)
			#if defined(ARDUINO_RASPBERRY_PI_PICO_W) || defined(ARDUINO_RASPBERRY_PI_PICO)
				//GPS_SERIAL_CLS SerialPIO(NOPIN, GPS_RX_PIN, 128);
...
void ckb(void *cbdata, int key) {
		...
		debug_serial_port->printf("Consumer: %02x %s \n", key, state ? "DOWN" : "UP");
...
void kb(void *cbdata, int key) {
		...
		// Modifier key has been pressed.  Set hold key flag and bit mask and exit, then only process regular keys until modifier key is let up
				switch (key) {
						case 0xe0: mod_key = KeyModifier::L_CTRL;  _heldKey = key; break;
						case 0xe4: mod_key = KeyModifier::R_CTRL;  _heldKey = key; break;
						case 0xe2: mod_key = KeyModifier::L_ALT;   _heldKey = key; break;
						case 0xe6: mod_key = KeyModifier::R_ALT;   _heldKey = key; break;
						case 0xe3: mod_key = KeyModifier::L_META;  _heldKey = key; break;
						case 0xe1: mod_key = KeyModifier::L_SHIFT; _heldKey = key; break;
						case 0xe5: mod_key = KeyModifier::R_SHIFT; _heldKey = key; break;						
				}
				inf.keys[2] = key;
...
#if defined(FEATURE_BT_KEYBOARD) && defined(HARDWARE_ESP_DEV)
	// Override default BT memory management - fix for controller not starting due to unreleased emeory? arduino-ep32 3.3.7 and maybe earlier also.
	bool btInUse(void) {
	return true;  // Keep BT memory
	}
#endif

void initialize_bt_keyboard(){  // init the BT 4.2 stack for ESP32-WROOM-32 for BLE and BT Classic
	#if defined(FEATURE_BT_KEYBOARD)
			...
			if (bt_keyboard.setup(pairing_handler, keyboard_connected_handler, keyboard_lost_connection_handler)) {  // Must be called once
				...
				debug_serial_port->println("bt_keyboard.setup: Failed to start BT");
			}
		#endif
...
// before it fills up.  What is a byte wasted between friends?
//
//  This queue is shared between the BT keyboard and select Touch buttons
//
////////////////////////////////////////////////////////////////////////
...
void initialize_display() {
		...
		if (LCD_COLUMNS < 9) {
			lcd_center_print_timed("K3NGKeyr", 0, 4000);
				...
				if (LCD_ROWS > 3) lcd_center_print_timed(CODE_VERSION, 3, 4000);      // display the code version on the fourth line of the display
			#endif
...
void memX_key(uint16_t key_ID, uint8_t memory_number) {
	...
	//debug_serial_port->print(F("  Memory = ")); debug_serial_port->println(memory_number);

	if (btn[key[key_ID].btn_idx].len == 1) {  // short press
		BtnX_active = 0;  // Do not hold state on
		key[key_ID].hold = false;
		queueadd(PS2_F1 + memory_number - 1);   // add char to the queue
	} else {  // long press
		BtnX_active = key_ID;  // Hold button state ON
		key[key_ID].hold = true;
...
// Detects if a button is pressed and assign a button ID to it.
		...
		void check_touch_buttons(void * pvParameters) {
			...
			debug_serial_port->println(F("\ncheck_touch_buttons: Starting Waiting-for-Init Loop"));
			while (start_task < 6) {
				myDelay(200);
				debug_serial_port->println(F("check_touch_buttons: Waiting-for-Init to finish"));   // wait until config settings are read into memory
			}			
			debug_serial_port->println(F("check_touch_buttons: Proceeeding with Task"));
			...
			for(;;) {
				...
				//debug_serial_port->println("check_bt_keyboard: Start");
					...
					do {
						...
						// Since there is a timeout, the content of the buffer should be ignored. There is a immediate return when wait is false.
...
void setup_esp()
{
	...
	// ------------------ TASK_MONITOR ----------------------------------------------------------------------------
		...
		xTaskCreate(TaskMonitor, "Main_Loop Monitor", 1000, NULL, 1, &xHandle_Monitor);
	#endif
...
Use ps() task status list to see what tasks are running

Each task will loop around waiting for its flag to start in the correct order
...
#endif
{
	...
	for(;;)
	{
		...
		uint32_t current_time = millis();
		    ...
		    if (current_time - last_ps_time >= ps_time) {
				//ps();
				last_ps_time = current_time;
				//printFreeMemory();		
				//int getFreeHeap = rp2040.getFreeHeap();
				//debug_serial_port->print(F("Main Free Heap:")); debug_serial_port->println(getFreeHeap);		
				//uint16_t val;
				//val = mcp23017.readPort(MCP23017Port::A);
				//debug_serial_port->print(F("MCP23017 Port A read:"));debug_serial_port->print(val,HEX);
			}
		#endif

		// Need to contol the start order of all the tasks.  
		// Problem is the order and number running as a task can change.
		// Will use a variable "start_task" that is incremented here and tested by each task, if it is running.
		// At the start of each task it sits in a loop waiting for start_task to match it assigned priority number.
		current_time = millis();
