...
void BluetoothHCI::hci_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    case HCI_EVENT_PIN_CODE_REQUEST:
        ...
        DEBUGV("HCI_EVENT_PIN_CODE_REQUEST from %02X:%02X:%02X:%02X:%02X:%02X, replying with fixed PIN",
        ...
        gap_pin_code_response(address, "1234");
        break;

    case HCI_EVENT_USER_PASSKEY_NOTIFICATION:
        {
            // raw layout: [0]=event, [1]=len, [2-7]=bd_addr, [8-11]=passkey (uint32 LE, 000000-999999)
            uint32_t passkey = little_endian_read_32(packet, 8);
            Serial.printf("\n*** Bluetooth pairing passkey: %06lu -- type this on the keyboard and press Enter ***\n\n",
    ...
    case HCI_EVENT_AUTHENTICATION_COMPLETE:
        // raw layout: [0]=event, [1]=len, [2]=status, [3-4]=conn handle
        DEBUGV("HCI_EVENT_AUTHENTICATION_COMPLETE status=0x%02X", packet[2]);
        break;

    case HCI_EVENT_LINK_KEY_NOTIFICATION:
        // raw layout: [0]=event, [1]=len, [2-7]=bd_addr, [8-23]=link_key, [24]=link_key_type
        DEBUGV("HCI_EVENT_LINK_KEY_NOTIFICATION link_key_type=%d", packet[24]);
        break;

    case HCI_EVENT_ENCRYPTION_CHANGE:
        // raw layout: [0]=event, [1]=len, [2]=status, [3-4]=conn handle, [5]=enabled
        DEBUGV("HCI_EVENT_ENCRYPTION_CHANGE status=0x%02X enabled=%d", packet[2], packet[5]);
    ...
    case HCI_EVENT_DISCONNECTION_COMPLETE:
        ...
        // raw layout: [0]=event, [1]=len, [2]=status, [3-4]=conn handle, [5]=reason
        DEBUGV("HCI Disconnected, reason=0x%02X", packet[5]);
        break;
